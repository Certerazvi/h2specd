...
const (
	Default						uint32 = 0x0
	PrefaceTestCase                 		uint32 = 0x1
	InvalidHeaderTestCase				uint32 = 0x2
	IllegalRST_STREAMFrameWhileIdleTestCase 	uint32 = 0x3
	SelfDependingPriorityFrameTestCase		uint32 = 0x4
	CloseConnAfterGoAwayFrameTestCase		uint32 = 0x5
	DiscardingUnknownFramesTestCase 		uint32 = 0x6
	DataFrameWith0x0StreamIdentTestCase		uint32 = 0x7
	RST_FRAMEWith0x0StreamIdentTestCase		uint32 = 0x8
	SettingsACKTestCase				uint32 = 0x9
	PingFrameReplyTestCase				uint32 = 0xA
	NonZeroIdentPingFrameTestCase			uint32 = 0xB
	PingFrameWithLengthDiffFromEightTestCase	uint32 = 0xC
	GoAwayWithNonZeroStreamIdentTestCase		uint32 = 0xD
	ZeroFlowControlWindowIncrementTestCase		uint32 = 0xE
...
var ServerConn *http2serverConn
...
func (sc *http2serverConn) serve() {
	...
	if TestNo == PrefaceTestCase {
		...
		frameSize = sc.srv.maxReadFrameSize()
	}

	msg := http2frameWriteMsg{
		write: http2writeSettings{
			{http2SettingMaxFrameSize, frameSize},
	...
	if TestNo == IllegalRST_STREAMFrameWhileIdleTestCase {
		time.Sleep(3 * time.Second)
		sc.Framer().WriteRSTStream(1, Http2ErrCodeCancel)
		sc.Framer().WriteWindowUpdate(1, 100)
	}
